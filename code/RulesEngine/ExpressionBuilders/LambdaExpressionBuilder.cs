    ...
    internal sealed class LambdaExpressionBuilder : RuleExpressionBuilderBase
    {
        ...
        internal override RuleFunc<RuleResultTree> BuildDelegateForRule(Rule rule, RuleParameter[] ruleParams)
        {
            try
            {
                ...
                var ruleDelegate2 = _ruleExpressionParser.Compile<dynamic>(rule.Expression, ruleParams);
                return Helpers.ToResultTree(_reSettings, rule, null, ruleDelegate2);
            }
            catch (Exception ex)
            {
                ...
                dynamic func(object[] param) => false;
